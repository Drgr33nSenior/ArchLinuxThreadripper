...
tar() { [[ ${tar_failure:-false} == false ]] || return 42; printf '%s\n' "$archive_entries"; }
...
(
  common::validate_esp_pair() { COMMON_ESP_A_DEVICE=/dev/synthetic1; COMMON_ESP_A_DISK=/dev/synthetic; }
  lsblk() { [[ ${partition_failure:-false} == false ]] || return 42; printf '%s\n' "$partition_number"; }
