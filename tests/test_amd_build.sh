...
if (ws_calculate_jobs 24000 48 24 2048 16384 8192 1) >/dev/null 2>&1; then
  echo 'insufficient RAM was accepted' >&2
  exit 1
fi
if (ws_calculate_jobs 65536 48 24 0 16384 8192 1) >/dev/null 2>&1; then
  echo 'zero per-job memory was accepted' >&2
  exit 1
fi
...
for bad in "$flags -O3" "$flags -ffast-math" "$flags -march=native" "${flags/-O2/-Os}"; do
  if (ws_native_flags "$bad") >/dev/null 2>&1; then
    echo 'unsafe or ambiguous CPU flags accepted' >&2
    exit 1
  fi
done
...
if ws_hardware_assert_json "$pci" "$(jq '.[0:1]' <<<"$agents")" 2; then
  echo 'one visible GPU accepted' >&2
  exit 1
fi
...
[[ ! -e $work/bad-plan ]]
if (
  export HSA_OVERRIDE_GFX_VERSION=99.0.0
  ws_rocm_unfiltered
