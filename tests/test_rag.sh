...
curl() {
  ...
  while (($#)); do
    arg=$1
    shift
    if [[ $arg == --output ]]; then destination=$1; shift; else url=$arg; fi
  done
...
if (fail_download=true; ws_rag_stage_models "$scratch/download-failure") >"$scratch/failure.log" 2>&1; then exit 1; fi
[[ -d $scratch/download-failure && ! -e $scratch/download-failure/provenance.json ]]
if (corrupt_download=true; ws_rag_stage_models "$scratch/corruption") >"$scratch/corruption.log" 2>&1; then exit 1; fi
