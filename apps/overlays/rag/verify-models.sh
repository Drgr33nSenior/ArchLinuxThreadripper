...
(($# == 2)) || {
  printf 'Usage: verify-models.sh MODEL_DIRECTORY SHA256SUMS\n' >&2
  exit 2
}
directory=$1
manifest=$2
[[ -d $directory && ! -L $directory && -s $manifest ]] || {
  printf 'Missing RAG models or checksums\n' >&2
  exit 1
}
count=0
seen='|'
while read -r expected relative extra; do
  ...
  case $seen in *"|$relative|"*)
    printf 'RAG checksum manifest has duplicate entries\n' >&2
    exit 1
    ;;
  esac
  seen="${seen}${relative}|"
  ...
  [[ ${actual%% *} == "$expected" ]] || {
    printf 'RAG checksum mismatch: %s\n' "$relative" >&2
    exit 1
  }
  count=$((count + 1))
done <"$manifest"
((count == 10)) || {
  printf 'Incomplete RAG model manifest\n' >&2
  exit 1
}
