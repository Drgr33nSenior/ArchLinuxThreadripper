...
k3s_allowed_key() {
  case "$1" in
    ...
    STEAM_HEADLESS_BASE_IMAGE|STEAM_HEADLESS_REVIEWED_SOURCE|SUNSHINE_VERSION|SUNSHINE_COMMIT|SUNSHINE_DEBIAN_SHA256|SUNSHINE_MESA_DEBIAN_VERSION) return 0 ;;
...
k3s_validate_value() {
  ...
  case "$key" in
    ...
    SUNSHINE_MESA_DEBIAN_VERSION) [[ "$value" =~ ^[0-9][a-zA-Z0-9.+~:-]+$ ]] || k3s_die "$key must be a Debian package version" ;;
...
k3s_load_config() {
  ...
  unset STEAM_HEADLESS_BASE_IMAGE STEAM_HEADLESS_REVIEWED_SOURCE SUNSHINE_VERSION SUNSHINE_COMMIT SUNSHINE_DEBIAN_SHA256 SUNSHINE_MESA_DEBIAN_VERSION
