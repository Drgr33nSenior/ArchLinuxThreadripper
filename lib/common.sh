...
common::bootnum_for_label() {
  ...
  inventory=$(LC_ALL=C efibootmgr -v) || { common::warn 'UEFI inventory failed'; return 2; }
  common::validate_boot_inventory <<< "$inventory" || { common::warn 'Malformed UEFI inventory'; return 2; }
  entries=$(common::boot_entries_for_label "$label" <<< "$inventory") || return 2
  count=$(awk 'NF {n++} END {print n+0}' <<< "$entries")
  [[ $count != 0 ]] || { common::warn "UEFI entry missing: $label"; return 1; }
  [[ $count == 1 ]] || { common::warn "UEFI entry not unique: $label"; return 2; }
  if [[ -n $partuuid || -n $loader ]]; then
    [[ -n $partuuid && -n $loader ]] || { common::warn 'UEFI validation requires partition and loader together'; return 2; }
    path=${entries#*$'\t'}
    ...
    [[ $path == *"hd("*",gpt,$partuuid,"* && $path == *"/file($loader)"* ]] \
      || { common::warn "UEFI partition or loader mismatch: $label"; return 2; }
  fi
  printf '%s\n' "${entries%%$'\t'*}"
}
...
common::validate_esp_pair() {
  ...
  for mountpoint in /efi /efi2; do
    ...
    [[ -z $extra && $kind == part && $parttype == c12a7328-f81f-11d2-ba4b-00a0c93ec93b \
      ...
      && $parent =~ ^[a-zA-Z0-9_.!-]+$ ]] || { common::warn "Not a GPT ESP partition: $device"; return 1; }
    parent=$(readlink -f -- "/dev/$parent") || return 1
    parent_type=$(lsblk --nodeps --noheadings --raw --output TYPE,PTTYPE "$parent") || return 1
    [[ $parent_type == 'disk gpt' ]] || { common::warn "ESP parent is not a GPT disk: $parent"; return 1; }
    case $expected in
      UUID=*)
        uuid=$(lsblk --nodeps --noheadings --raw --output UUID "$device") || return 1
        [[ -n $uuid && $uuid != *$'\n'* && $expected == "UUID=$uuid" ]] || { common::warn "ESP UUID differs from fstab: $mountpoint"; return 1; } ;;
      PARTUUID=*)
        expected=$(printf '%s' "$expected" | tr '[:upper:]' '[:lower:]')
        [[ $expected == "partuuid=$partuuid" ]] || { common::warn "ESP PARTUUID differs from fstab: $mountpoint"; return 1; } ;;
      *) common::warn "ESP fstab source must use UUID or PARTUUID: $mountpoint"; return 1 ;;
    esac
    [[ $partuuid != "$prior_part" && $parent != "$prior_disk" ]] \
      || { common::warn 'Backup ESP must be a distinct partition on another disk'; return 1; }
    ...
    prior_part=$partuuid; prior_disk=$parent; side=B
  done
}
