...
ws_session_template_check() {
  ...
  if [[ "$deployment" == "$SESSION_AI_DEPLOYMENT" ]]; then expected="${WORKSTATION_SESSION_AI_TEMPLATE_SHA256:-}";
...
ws_session_gpu_free() {
  ...
  for pid in /proc/[0-9]*; do
    ...
    for fd in "$pid"/fd/*; do
      [[ -L "$fd" ]] || continue
      number="$(stat -Lc '%F:%t:%T' -- "$fd" 2>/dev/null)" || { [[ ! -L "$fd" ]] && continue; ws_die 'cannot inspect a live process descriptor'; }
...
ws_session_build_gate() {
  ...
  case "$action" in
    inhibit) (umask 022; printf 'managed builds inhibited during gaming or an incomplete transition\n' > "$directory/build-inhibit") ;;
...
ws_session_switch() (
  ...
  ws_require_arch; ws_require_root; ws_session_config_validate; ws_build_config_validate
  ...
  case "$mode" in
    ...
    restore)
      if [[ "$(jq -r .previous.ai <<< "$state")" == 1 ]]; then desired="$SESSION_AI_DEPLOYMENT";
