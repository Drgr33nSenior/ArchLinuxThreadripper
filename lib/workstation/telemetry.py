...
def render(args):
    ...
    require(total <= args.reserve_mib, "telemetry limits exceed TELEMETRY_RESERVE_MIB")
    ...
    evidence = {"schema": 1, "status": "generated-not-deployed" if enabled else "disabled-not-deployed",
...
def verify(directory):
    ...
    require(evidence.get("schema") == 1 and evidence.get("status") in {"generated-not-deployed", "disabled-not-deployed"},
