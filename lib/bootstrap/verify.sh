...
bootstrap_verify_file() {
  local path=$1
  [[ -s "$BOOTSTRAP_TARGET$path" ]] || { bootstrap_die "required file is absent or empty: $path"; return 1; }
}

bootstrap_verify_uki_signature() {
  local uki=$1 cert=$2
  sbverify --cert "$cert" "$uki" >/dev/null \
    || { bootstrap_die "UKI is not signed by the configured Secure Boot db certificate: $uki"; return 1; }
}
...
bootstrap_verify_boot_order() {
  ...
  command -v efibootmgr >/dev/null 2>&1 || { bootstrap_die 'efibootmgr is required for UEFI BootOrder verification'; return 1; }
  ...
  [[ $actual == "$expected" || $actual == "$expected,"* ]] \
    || { bootstrap_die "UEFI BootOrder does not begin $expected"; return 1; }
}
...
bootstrap_verify() {
  ...
  command -v sbverify >/dev/null 2>&1 || { bootstrap_die 'sbverify is required for Secure Boot verification'; return 1; }
  [[ -r $cert ]] || { bootstrap_die "configured Secure Boot db certificate is not readable: $cert"; return 1; }
  ...
  [[ $(awk '{print $2}' <<< "$state") == P ]] \
    || { bootstrap_die 'root emergency authentication is locked or has no password'; return 1; }
  ...
  [[ -x "$BOOTSTRAP_TARGET$helper" ]] \
    || { bootstrap_die 'UKI sync helper is not executable'; return 1; }
  grep -Fqx 'When = PostTransaction' "$BOOTSTRAP_TARGET$hook" \
    || { bootstrap_die 'UKI sync hook is not post-transaction'; return 1; }
  grep -Eq "^Exec = $helper( --sign)?$" "$BOOTSTRAP_TARGET$hook" \
    || { bootstrap_die 'UKI sync hook does not use the approved helper'; return 1; }
  ...
  grep -Fqx 'HOOKS=(base systemd autodetect microcode modconf kms keyboard sd-vconsole block mdadm_udev sd-encrypt filesystems fsck)' "$BOOTSTRAP_TARGET/etc/mkinitcpio.conf" \
    || { bootstrap_die 'mkinitcpio hook order differs from the supported storage path'; return 1; }
  grep -Fq 'x-initrd.attach' "$BOOTSTRAP_TARGET/etc/crypttab" || { bootstrap_die 'crypttab is missing x-initrd.attach'; return 1; }
  grep -Fq 'fido2-device=auto' "$BOOTSTRAP_TARGET/etc/crypttab" || { bootstrap_die 'crypttab is missing FIDO2 autodetection'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/etc/crypttab" "$BOOTSTRAP_TARGET/etc/crypttab.initramfs" \
    || { bootstrap_die 'runtime and initramfs crypttab policies differ'; return 1; }
  grep -Fq 'discard' "$BOOTSTRAP_TARGET/etc/crypttab.initramfs" || { bootstrap_die 'initramfs crypttab is missing the approved discard policy'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/efi/EFI/Linux/arch-linux.efi" "$BOOTSTRAP_TARGET/efi2/EFI/Linux/arch-linux.efi" \
    || { bootstrap_die 'stable UKI copies differ between ESPs'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/efi/EFI/Linux/arch-linux-lts.efi" "$BOOTSTRAP_TARGET/efi2/EFI/Linux/arch-linux-lts.efi" \
    || { bootstrap_die 'LTS UKI copies differ between ESPs'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/efi/EFI/Linux/arch-recovery.efi" "$BOOTSTRAP_TARGET/efi2/EFI/Linux/arch-recovery.efi" \
    || { bootstrap_die 'recovery UKI copies differ between ESPs'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/efi/EFI/BOOT/BOOTX64.EFI" "$BOOTSTRAP_TARGET/efi2/EFI/BOOT/BOOTX64.EFI" \
    || { bootstrap_die 'fallback UKI copies differ between ESPs'; return 1; }
  cmp -s "$BOOTSTRAP_TARGET/efi/EFI/Linux/arch-recovery.efi" "$BOOTSTRAP_TARGET/efi/EFI/BOOT/BOOTX64.EFI" \
    || { bootstrap_die 'fallback UKI is not the signed recovery UKI'; return 1; }
  for uki in \
