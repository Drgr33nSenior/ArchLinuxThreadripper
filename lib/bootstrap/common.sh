...
bootstrap_require_root() {
  [[ ${EUID:-$(id -u)} -eq 0 ]] || {
    bootstrap_die 'run this action as root from the Arch ISO'
    return 1
  }
}
...
bootstrap_require_tty() {
  [[ -t 0 && -t 1 ]] || {
    bootstrap_die 'a controlling terminal is required for passphrase and serial confirmation'
    return 1
  }
}
...
bootstrap_require_arch_iso() {
  command -v pacstrap >/dev/null || {
    bootstrap_die 'pacstrap is missing; boot the official Arch installation ISO'
    return 1
  }
  command -v arch-chroot >/dev/null || {
    bootstrap_die 'arch-chroot is missing; boot the official Arch installation ISO'
    return 1
  }
  [[ -r /etc/arch-release ]] || {
    bootstrap_die 'this is not an Arch environment'
    return 1
  }
  [[ -d /run/archiso ]] || {
    bootstrap_die 'the destructive installer must run from an Arch live ISO'
    return 1
  }
}
...
bootstrap_require_uefi() {
  ...
  [[ -d "$sysfs_root/firmware/efi/efivars" ]] || {
    bootstrap_die 'UEFI boot is required; CSM/legacy boot is not supported'
    return 1
  }
}
...
bootstrap_require_test_vm() {
  local virtualizer
  virtualizer=$(systemd-detect-virt --vm) || {
    bootstrap_die 'VM test profile requires QEMU'
    return 1
  }
  [[ $virtualizer == qemu || $virtualizer == kvm ]] ||
    {
      bootstrap_die 'VM test profile requires QEMU/KVM'
      return 1
    }
  [[ -r /sys/class/dmi/id/sys_vendor && $(</sys/class/dmi/id/sys_vendor) == QEMU ]] ||
    {
      bootstrap_die 'VM test profile requires the QEMU DMI identity'
      return 1
    }
}
...
bootstrap_validate_by_id_path() {
  local path=$1
  [[ $path == /dev/disk/by-id/* ]] || {
    bootstrap_die "disk must be an absolute /dev/disk/by-id path: $path"
    return 1
  }
  [[ ${path##*/} == nvme-* ]] || {
    bootstrap_die "disk must use a persistent NVMe by-id path: $path"
    return 1
  }
  [[ $path != *'..'* && $path != *$'\n'* ]] || {
    bootstrap_die "unsafe disk path: $path"
    return 1
  }
  [[ -L $path ]] || {
    bootstrap_die "disk by-id symlink does not exist: $path"
    return 1
  }
}
...
bootstrap_disk_serial() {
  ...
  [[ -n $serial ]] || {
    bootstrap_die "cannot determine serial for $disk"
    return 1
  }
  printf '%s\n' "$serial"
}
...
bootstrap_part_path() {
  ...
  inventory=$(lsblk -nrpo NAME,PARTN "$disk") || {
    bootstrap_die "partition inspection failed: $disk"
    return 1
  }
...
bootstrap_assert_safe_target() {
  ...
  [[ $BOOTSTRAP_TARGET == /mnt ]] || {
    bootstrap_die 'installation target is intentionally fixed at /mnt'
    return 1
  }
  inventory=$(findmnt -rn -o TARGET) || {
    bootstrap_die 'mount inventory failed; cannot prove /mnt is unused'
    return 1
  }
  [[ -n $inventory ]] || {
    bootstrap_die 'mount inventory is unexpectedly empty'
    return 1
  }
